...
# but we need '/home/me/bar/mysymlink'
    return _coconut_tail_call((os.path.abspath), (os.path.expanduser)(Path(path).as_posix()))

def get_stdin_args():
    if sys.stdin.isatty():
        stdin_args = []
    else:
        stdin_args = (list)(filter(None, sys.stdin.read().splitlines()))
    return stdin_args
...
class Selection:
    ...
    def _read_items(self):
        try:
            ...
            text = ''
            self.clear()
        return _coconut_tail_call(text.splitlines)
    ...
    def add(self, items):
        ...
        all_items = map(normalize_path, filter(None, _coconut.itertools.chain.from_iterable((_coconut_func() for _coconut_func in (lambda: old_items, lambda: items)))))
# remove duplicates
        unique_items = (list)(((dict)(map(lambda x: (x, None), all_items))).keys())
        self._write_items(unique_items)

    def remove(self, items):
        old_items = self._read_items()
        abs_items = (list)(map(normalize_path, items))
        all_items = filter(lambda x: x not in abs_items, old_items)
...
def main():
    ...
    if args:
        ...
        _coconut_match_to = cmd
        _coconut_case_check_0 = False
        if _coconut_match_to == '+':
